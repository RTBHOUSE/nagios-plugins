...
#  [0.1  - Nov 8, 2013]  First beta verion check_atop_log.pl
#
...
my $now=time ;
my $atop_inerval;
my ($esec,$emin,$ehour,$emday,$emon,$eyear,$ewday,$eyday,$eisdst) = gmtime($now-60);

# Calculate atop interval ( meybe it is better way ? ) 
# from last cpu stats
$nlib->verb("Traying to calculate atop interveal from log");
my $cmd="atop -r -b $ehour:$emin -e $ehour:$emin -P CPU";
$nlib->verb("Looking for atop inteval in command:".$cmd);
#print Dumper($cmd);
my $atop_total_cpu_raw=`$cmd`;
if ( $? != 0 ) { print "CRITICAL ERROR - atop command failed: $cmd\n"; exit $ERRORS{'CRITICAL'}; }
my @atop_out=split(/\s+/,$atop_total_cpu_raw);
if (defined $atop_out[5]){
  $atop_inerval=$atop_out[5];
  $nlib->verb("Calculated atop interveal: ".$atop_inerval);
}else{
  $nlib->verb("Failed to calculate atop interveal, setting value to 60");
  $atop_inerval=60;
}

#print Dumper($atop_inerval);
($esec,$emin,$ehour,$emday,$emon,$eyear,$ewday,$eyday,$eisdst) = gmtime($now-$atop_inerval);
my ($bsec,$bmin,$bhour,$bmday,$bmon,$byear,$bwday,$byday,$bisdst) = gmtime($now-($atop_inerval*2));


# create hash table variaiables froam atopsar ( '-f', '-l' only in new atop ) 
my @atopsarswitches=('-c','-p','-P','-m','-s','-d','-i','-I','-w','-W','-y','-Y','-u','-z','-Z','-k','-K','-U','-t','-T');
my $atopsarcmd="atopsar -a -x -S -b $bhour:$bmin -e $ehour:$emin";
my %atopsarvar;
foreach my $switch (@atopsarswitches) {
#  print Dumper("$atopsarcmd $switch"); 
  my $atopsr_raw=`$atopsarcmd $switch`;
...
#  print "atopsar_lines=".scalar @atopsar_lines;
  if (scalar @atopsar_lines == 2) {
    if ( $atopsar_lines[0] =~ /(.*)_(\S+)_/) {
      my $key_prefix=$2;
      my @line1=split(/\s+/,$1);
      my @line2=split(/\s+/,$atopsar_lines[1]);
      for(my $i = 1; $i < scalar @line1; $i++) {
      ...
      print "CRITICAL ERROR - atopsar parse problem, unparsable report\n"; exit $ERRORS{'CRITICAL'};
    }
  }elsif (scalar @atopsar_lines > 2 ) {
    if ( $atopsar_lines[0] =~ /(.*)_(\S+)_/) {
      my $key_prefix=$2;
      my @line1=split(/\s+/,$1);
      for(my $x = 1; $x < scalar @atopsar_lines; $x++) {
        my @line2=split(/\s+/,$atopsar_lines[$x]);
        for(my $i = 2; $i < scalar @line1; $i++) {
          if (defined $line2[1]){
...
#  print Dumper(@atopsar_lines);
}
#print Dumper(\%atopsarvar);

while ( (my $key,my $val) = each(%atopsarvar)){
  if ( $key =~ /\/s/ ) {
   $nlib->add_knownvar($key,$key,"GAUGE","");
   $nlib->add_data($key,$val);
   $nlib->verb("Adding nev viaribale in /s :".$key."=".$val);
  }elsif ( $key =~ /%/ ) {
   $nlib->add_knownvar($key,$key,"GAUGE","%");
   $nlib->add_data($key,$val);
   $nlib->verb("Adding nev viaribale in %  :".$key."=".$val);
  }else{
   $nlib->add_knownvar($key,$key,"GAUGE","");
   $nlib->add_data($key,$val);
   $nlib->verb("Adding nev viaribale       :".$key."=".$val);
  }  
}
