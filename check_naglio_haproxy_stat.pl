...
# ============================== SUMMARY =====================================
#
# Program : check_naglio_haproxy_stat.pl
...
#
# This is Naglio Haproxy Stats plugin. It parse output of CSV status pages genereated 
# by stats haproxy backend. It sums stats from multiple backends.
#
...
#
#
# Setup haproxy to generate stats, for each running proces stats on diffrent port
#
#
# ... 
#listen  stats-1  *:1234 
...
#        stats refresh 5s
#        bind-process 1  
#        acl valid_src src 192.169.0.0/24 127.0.0.0/8 # Nagios ip should be included 
#        block if ! valid_src
#
#listen  stats-2  *:1234 
#        log global # logujemy polacznia tam gdzie wsazuje global
#        # nie logujemy z nagiosow  nie dziala : monitor-net  172.16.0.0/12
#        stats enable   
#        stats uri /      
#        stats refresh 5s
#        bind-process 2  
...
# 
#
# Then in nagios run command:
#
#   /usr/local/bin/check_naglio_haproxy_stat.pl  -H <haproxy_ip>  -U / -A  -p 7654 -p 7655 -p 7656 -p ....
#
#            
...
#  [0.1  - May 15, 2014]  First beta verion check_naglio_haproxy_stat.pl
#
...
my $o_rprefix='';
my $o_host=undef ;              # Hostname or ip to snmp connect
my @o_ports    =()    ;         # Port  list
my $o_url=      undef;          # url for haproxy stats
...
sub p_version { print "check_naglio_haproxy_stat version : $Version\n"; }

sub print_usage_line {
   print "Usage: $0 [-v [debugfilename]] [-a <statistics variables> -w <variables warning thresholds> -c <variables critical thresholds>] [-A <performance output variables>] [-f] [-T <timeout>] [-V] [-P <previous performance data in quoted string>] [-o <threshold specification with name or pattern>] -H <host_address> -U <URL> -p <port> \n";
}
...
sub help {
   ...
   print "Check Naglio Haproxy Stat",$Version,"\n";
   print "by Marek Grzybowski - marek(at)grzybowski.waw.pl\n\n";
   print "This is Haproxy stats monitoring plugin, it parses CSV genereted by\n";
   print "haproxy stats backend. any of parsed Value could be checked by numeric tresholds\n";
...
EXAMPLE:
   ...
   /usr/local/bin/check_naglio_haproxy_stat.pl  -H a11.creativecdn.net -U / -A  -p 1234 -p 1235

  Prints performce data from stats backends on port 1234 i 1235. You can select from this list
  what to monitor 


  /usr/local/bin/check_naglio_haproxy_stat.pl  -H a11.creativecdn.net -U / -A  -p 7654 -p 7655 \
  -o 'PATTERN:.*hrsp_5xx_rate_ps,WARN:>10,CRIT:>50'
  
  Will check if http 5xx resonce rate on any frontend or backend are less than 10 per second
...
# This variable will be used to generate temporary file 
my $argv_str=join('_',@ARGV);
$argv_str = md5_base64( $argv_str ) ;
$argv_str =~ s/[^a-zA-Z0-9]/x/g ;

my $nlib = Naglio->lib_init('plugin_name' => 'check_atop_log.pl',
			    'plugins_authors' => 'Marek Grzybowski',
			    'plugin_description' => 'Atop Log Plugin for Nagios',
			    'usage_function' => \&print_usage,
                            'enable_long_options' => 1,
                            'enable_rate_of_change' => 1);

check_options($nlib);
$nlib->verb("check_atop_log.pl plugin version ".$Version);
...
my $save_file="/tmp/check_naglio_haproxy_stats_".$login."_".$argv_str.'.perl';

my %result=();
my %old_result=();

#  print Dumper(@o_ports);

foreach my $port  (@o_ports) {
  my $http_get;
  my @csv;
  my @col_names;  # table of clumn names
  my $full_url;
  $full_url="http://".$o_host.":".$port.$o_url.";csv";
  $http_get=get($full_url);
  if ( ! defined $http_get) { printf "Cannot get from: ". $full_url . "\n";  exit $ERRORS{"UNKNOWN"}  }
  my $timenow=time();
  ${$result{'time'}}{'timenow'}=$timenow;
  @csv = split /\n/, $http_get;
  @col_names = split /,/, $csv[0];
  foreach my $line ( @csv[1 .. $#csv] ){
    my @fields;
    @fields= split /,/, $line;
    my $row_name;
    $row_name=$fields[0].'_'.$fields[1] ;
    $row_name =~ s/[^a-zA-Z0-9]/_/g ;
    foreach my $f_nr (  2 .. $#fields ) {
      if ( looks_like_number($fields[$f_nr])) {
        ${$result{$row_name}}{$col_names[$f_nr]} += $fields[$f_nr] ;
      }
    }
  }
...
#  print Dumper(\%result);
   ...
   #print Dumper(%old_result);  # entire result
   #print Dumper(%result);  # entire result
   # save current run 
   open my $out, '>', $save_file or die $!;
     local $Data::Dumper::Purity = 1;
     print {$out} Data::Dumper->Dump([\%result]);
   close $out;


###
#  Add counters to monitor and rates
###
foreach my $row_name (keys %result){
  foreach my $col_name (keys %{$result{$row_name}}) {
    my $name;
    my $value;
    $name=$row_name.'_'.$col_name.'_count';
    $value=${$result{$row_name}}{$col_name};
    $nlib->verb("Adding new counter".$name."=".$value);
    $nlib->add_knownvar($name,$name,"COUNTER","");
    $nlib->add_data($name,$value);
    
    #
    # Calculate and monior rates 
    #
    if (defined ${$old_result{'time'}}{'timenow'}) {
      my @rate_list=( 'stot','bin','bout','dreq','dresp','ereq','econ','eresp','hrsp_1xx','hrsp_2xx','hrsp_3xx','hrsp_4xx','hrsp_5xx','hrsp_other','req_tot','cli_abrt','srv_abrt' );
      if ( grep( /^$col_name$/, @rate_list ) ) {
        my $delta=undef ;
        my $old_value;
        my $rate ;
        $delta=${$result{'time'}}{'timenow'} - ${$old_result{'time'}}{'timenow'};
        $old_value=${$old_result{$row_name}}{$col_name};
        if (defined $value && defined $old_value && $delta) {
          $rate=($value-$old_value)/$delta;
          if ($rate >= 0 ) {
             $rate=sprintf "%.2f" , $rate ;
             $name=$row_name.'_'.$col_name.'_rate_ps';
             $nlib->verb("Adding new rate:".$name."=".$rate);
             $nlib->add_knownvar($name,$name,"GAUGE","");
             $nlib->add_data($name,$rate);
          }
        }
      }
    }
