...
#  [0.1  - Mar 13, 2014] First beta verion check_sockstat.pl
#
...
my $nlib = Naglio->lib_init('plugin_name' => 'check_sockstat.pl',
			    'plugins_authors' => 'Marek Grzybowski',
			    'plugin_description' => 'Sockstat Plugin for Nagios',
			    'usage_function' => \&print_usage,
                            'enable_long_options' => 1,
                            'enable_rate_of_change' => 1);

check_options($nlib);
$nlib->verb("check_sockstat.pl plugin version ".$Version);
...
my %sockstat_val;
if ( $sockstat_raw  =~ /sockets: used (\d+)/ ) {
  $sockstat_val{"sockets_used"}=$1;
} else {
   print "CRITICAL ERROR - parse problem, no 'sockets: used ...' \n"; exit $ERRORS{'CRITICAL'};
}

if ( $sockstat_raw  =~ /TCP: inuse (\d+) orphan (\d+) tw (\d+) alloc (\d+) mem (\d+)/ ) {
  $sockstat_val{"TCP_inuse"}=$1;
  $sockstat_val{"TCP_orphan"}=$2;
  $sockstat_val{"TCP_tw"}=$3;
  $sockstat_val{"TCP_alloc"}=$4;
  $sockstat_val{"TCP_mem"}=$5;
} else {
   print "CRITICAL ERROR - parse problem, no 'TCP: inuse ...' \n"; exit $ERRORS{'CRITICAL'};
}

if ( $sockstat_raw  =~ /UDP: inuse (\d+) mem (\d+)/ ) {
  $sockstat_val{"UDP_inuse"}=$1;
  $sockstat_val{"UDP_mem"}=$2;
} else {
   print "CRITICAL ERROR - parse problem, no 'UDP: inuse ...' \n"; exit $ERRORS{'CRITICAL'};
}

if ( $sockstat_raw  =~ /UDPLITE: inuse (\d+)/ ) {
  $sockstat_val{"UDPLITE_inuse"}=$1;
} else {
   print "CRITICAL ERROR - parse problem, no 'UDPLITE: inuse ...' \n"; exit $ERRORS{'CRITICAL'};
}

if ( $sockstat_raw  =~ /RAW: inuse (\d+)/ ) {
  $sockstat_val{"RAW_inuse"}=$1;
} else {
   print "CRITICAL ERROR - parse problem, no 'RAW: inuse ...' \n"; exit $ERRORS{'CRITICAL'};
}

if ( $sockstat_raw  =~ /FRAG: inuse (\d+) memory (\d+)/ ) {
  $sockstat_val{"FRAG_inuse"}=$1;
  $sockstat_val{"FRAG_memory"}=$2;
} else {
   print "CRITICAL ERROR - parse problem, no 'FRAG: inuse ...' \n"; exit $ERRORS{'CRITICAL'};
}
...
if ( $tcp_mem_raw  =~ /(\d+)/ ) {
  $sockstat_val{"TCP_mem_usage_percent"}=$sockstat_val{"TCP_mem"}*100/$1;
} else {
   print "CRITICAL ERROR - parse problem, no '<number> ...' in /proc/sys/net/ipv4/tcp_mem \n"; exit $ERRORS{'CRITICAL'};
}


$cmd="cat /proc/sys/net/ipv4/tcp_max_orphans";
$nlib->verb("Looking for values in output of  command:".$cmd);
#print Dumper($cmd);
my $tcp_max_orphans_raw=`$cmd`;
if ( $? != 0 ) { print "CRITICAL ERROR - command failed: $cmd\n"; exit $ERRORS{'CRITICAL'}; }

if ( $tcp_mem_raw  =~ /(\d+)/ ) {
  $sockstat_val{"TCP_orphans_usage_percent"}=$sockstat_val{"TCP_orphan"}*100/$1;
} else {
   print "CRITICAL ERROR - parse problem, no '<number> ...' in /proc/sys/net/ipv4/tcp_max_orphans \n"; exit $ERRORS{'CRITICAL'};
}




while ( (my $key,my $val) = each(%sockstat_val)){
